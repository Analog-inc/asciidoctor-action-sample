= Asciidoctor action sample README
:revnumber: 1.0
:docname: Asciidoctor action sample
:author: Analog inc.
:organization: Analog inc.
:homepage: https://analog-inc.com/
// ドキュメント仕様
:asciidoctor:
:doctype: book
:lang: ja
:icons: font
:encoding: utf-8
:backend: html5
:docinfo: shared
// 画像を埋め込み コマンドから指定
// :data-uri:
// 目次
:toclevels: 3
// セクションにアンカーを作成
:sectanchors:
// セクションにリンクを作成
:sectlinks:
// 章番号を付けるか。属性を定義しておくだけで章番号が付けられる。
:sectnums:
// ソースコードをハイライトする。
:source-highlighter: coderay
// 画像やビデオリソース(videoは個別に出力先にコピーが必要)
// 直リンクの方が編集時に扱いやすいため、コメントアウト
// :imagesdir: ./images
// PDF周り(コマンドで指定するためコメントアウト)
// :pdf-fontsdir: ./fonts
// :pdf-style: ./themes/sample-theme.yml
// 親テーマをasciidoctor-pdfの物を使う
// :pdf-styledir: ./themes
// ダイアグラム生成のフォントを指定
:plantumlconfig: configs/plantuml.cfg

include::define-common.adoc[]
include::define-literal.adoc[]



ifndef::target-release[]

== 現在のステータス

link:https://github.com/Analog-inc/asciidoctor-action-sample/actions?query=workflow%3A%22Azure+Static+Web+Apps+CI%2FCD%22[image:https://github.com/Analog-inc/asciidoctor-action-sample/workflows/Azure%20Static%20Web%20Apps%20CI/CD/badge.svg[StatusBatch,role="right"]] +


== Azure static web appsの利用

本ナレッジはGithub ActionsのワークフローとAzure static web appsを利用して招待されたメンバーのみがweb上で閲覧できるような仕組みを整えています。

=== 本ナレッジのデプロイ

「/master」のブランチにpushすると +
link:https://sample.asciidoc.analog-inc.com[リリース後のマニュアルが閲覧できるAzure static web appsサイト] にデプロイされます。

=== PullRequest時のレビューについて

「/master」をベースブランチとしてPullRequestをすると、
上記それぞれに対応するステージング環境にレビュー用のサイトとしてデプロイされます。 +
レビュー用のサイトはPullRequestをクローズすると、クローズされ閲覧できなくなります。

レビュー用のデプロイ先は当該PullRequestにbotがコメントしますが、表示されない場合は
Actionsのログを確認し、「Build and Deploy Job」内部のログ「Build and Deploy」の下部に、「Visit your site at:」として記載されていますので、
そこからアクセスしてください。

=== 本ナレッジを閲覧するには
デプロイされた本ナレッジを閲覧するにはAzure portalのAzure static web appsの当該リソースで権限付与(ロール指定なし)が必要です。 +
Azure static web appsのロール画面で対象者のメールアドレスを入力し一人づつ招待リンクを生成してください。

=== Azure static web appsについて
link:https://docs.microsoft.com/ja-jp/azure/static-web-apps/[詳しくはこちら]
を参照して下さい。

=== 利用しているAzureのリソースグループについては
link:https://portal.azure.com/[こちら]
になります。

endif::[]




== 本ナレッジ(ドキュメント)作成の目的
ここでは本ナレッジの目的を説明します。
これらの目的を達成するために、Gitでのバージョン管理を行える、Asciidoc形式でドキュメント作成を行います。(参照: <<WHAT-ASCIIDOC>>)

=== 全体を見渡せるマニュアルの必要性
現在、全ての仕様を網羅し的確な説明がなされたマニュアルはありません。 +
{name-role-helpdesk}にて各権限事の仕様の違いについて、簡単に見分けがつき、 +
的確な案内をすぐに行うと共に、今後の仕様変更に追従が容易なナレッジの作成を目指します。

=== バージョン管理の必要性
マニュアルはシステムの更新と共に、更新され共存する必要があります。 +
また、お客様の不明点や、問題解決に必要なTipsの記載も欠かせません。 +
このため、いつ何のためにどんな修正が行われたか、Gitによるバージョン管理で追跡し、 +
必要であれば過去のバージョンを取り消すことも必要です。

=== スタイルの統一
従来のドキュメントエディタの多くは、更新が重なると適用したスタイルを維持することが難しくなります。 +
Asciidoc形式は記載方法がテキスト形式に限られるため、記載時にスタイルを切り離して記載することができます。 +
これにより、より大きな文章でも統一されたスタイルで提供することが可能です。

=== 表現の統一
従来のドキュメントエディタの多くは、文字列の定義や、繰り返し利用される記述の一括定義が行えず、 +
記載箇所毎の大量の表現ゆれ、表記ゆれに対応する必要がありました。 +
また、表記ゆれを確認するために検索を行うと大量の検索結果が表示されてしまい +
あえて別表現を使っている場合の抽出が困難でした。 +
Asciidoc形式は文字列の定義や、文書の参照取り込みが可能なため、これらの問題に対し適切な運用が可能です。

=== 各対象毎の出力
全ての仕様を網羅し的確な説明がなされたマニュアルがあれば、それから各権限向けのマニュアルを作りたいと考えるのは自然な発想です。 +
従来のドキュメントエディタの多くでも、そのような運用がなされていますが、更新が進むに連れて対象権限毎に表現を修正したドキュメントと、 +
最新の仕様が記載された網羅的なドキュメントのマージが難しくなり、表現ゆれや記載ミスの原因となっています。 +
Asciidoc形式はifdefディレクティブの利用により簡易的な出力制御が行えるため、 +
網羅的なドキュメントに全ての権限に対する記載を行うことができ、安定的に各権限のマニュアルをリリースすることが可能です。

=== リリース向け出力形式の最適化
現在、多くのシステムではPDFのマニュアルが配布されますが、 +
WebシステムではHTMLのマニュアルも多用されています。 +
HTMLのマニュアルは目次もサイドバー等に表示可能になりブラウザの機能と合わせて目的検索生が向上するためです。 +
従来のドキュメントエディタもほとんどの場合でPDF出力が可能になっていますが、 +
HTMLでの出力を行うと、HTMLの利便性を享受できないことが多く、最悪の場合レイアウト崩れ等が発生することが多々あります。 +
Asciidoc形式はPDF出力とHTML出力に対して、それぞれ最適な出力を提供します。

== AsciiDocに移行するデメリット
ここでは従来のドキュメントと、本ナレッジを比較した場合のデメリットを記載します。

=== 記載方法に慣れていない
今までOffice suite等のドキュメントエディタを利用していた場合、 +
多くの方が編集方法に戸惑うと考えられます。 +
ただしこれらの問題はドキュメントエディタが更新された場合にも当てはまります。 +
Asciidoc形式はMarkdown形式の様な簡易的な記載で行うことができる学習コストが低い記載方法の一つです。 +
従来のドキュメントエディタと異なり、スタイルを壊してしまうこともないため大きな問題にならないと考えています。

=== 編集履歴や校閲機能がない
Asciidoc形式はテキスト形式のため、編集履歴や校閲機能はありません。 +
これらを同じドキュメントエディタ上で利用していた場合は当初不便を感じると思われます。 +
Asciidoc形式では、本文中に出力されないコメントを記載することは可能です。 +
また、より確実なバージョン管理を行うためにGit-hubのプライベートリポジトリを利用することで、 +
Issueの管理による議論や、それに紐付けた更新など、多彩なバージョン管理等を行うことでカバー可能です。

=== 記載中にイメージがわかない
Asciidoc形式はテキスト形式のため、従来のドキュメントエディタと異なり、記載中はスタイルが適用されません。 +
しかし、現在は無料のテキストエディタや統合開発環境に(Atom, VS Code, IntelliJ IDEA, Android studio)等にプラグインをインストールすることで、 +
簡単にプレビューを表示しながら編集を行うことが可能です。

=== 特殊な記述ができない
Asciidoc形式は原則として技術本やマニュアルを記載するための汎用的な構成を重視されているため、 +
画像の上に画像を貼り付けたり、ある一箇所だけ特別なスタイルを変更することができません。 +
これらについては、更新が進むたびに表現方法がバラついてしまうことがなくなるため、 +
一長一短だと考えられます。


=== ドキュメントエディタで校閲や添削できない
Asciidoc形式は出力を行うとPDFやHTMLにて出力されるため、その状態での校閲や添削はできません。 +
特にPDFではページング制御とうがあるため、出力イメージ上で校閲や添削を行いたいと考えられます。 +
この点についてPDFは、Adobe Acrobat ReaderやAppleのプレビューアプリには注釈機能があるため、 +
直接の編集ではない校閲や添削は可能です。 +
また、HTMLについてもスクリーンショット等にマークアップすることが多くのブラウザやOSで可能になっているため、 +
編集者以外の校閲や添削に問題はありません。

[[WHAT-ASCIIDOC]]
== AsciiDocについて

AsciiDocは軽量マークアップ言語の一つで可読文書記述形式です。 +
出力もHTML, PDF, ePubなどが行えるため出版や開発環境でのナレッジの共有まで様々な用途で利用することが可能です。

* link:https://asciidoc.org/[AsciiDoc 公式サイト]

* AsciiDocをマニュアルに利用している例
** link:https://www.science.co.jp/case/yamaha/github_asciidoc/[YAMAHA様事例]
* AsciiDocを執筆に利用している例
** O’Reilly Mediaの書籍の一部(Welcome kitとして公開していました)
* AsciiDocをナレッジに利用している例
** link:https://github.com/github/markup[GitHub](READMEやWikiに利用するオープンソースマークアップライブラリに採用されています)

=== AsciiDocの記述方法

AsciiDocは軽量マークアップ言語でメジャーなフォーマットであるmarkdownと比較し、
同様の記述容易性を供えつつもより高度な文書の論理構造を構築することが可能です。 +
記載方法の簡単な説明は以下のリンクをご参照ください。

* link:https://takumon.github.io/asciidoc-syntax-quick-reference-japanese-translation/[Asciidoctor 文法クイックリファレンス(日本語訳)]



=== Asciidoctorについて

Asciidoctorは、AsciiDoc形式のための包括的なツールで、AsciiDoc形式をHTML5、DocBook、PDFなどの形式に変換します。

==== Asciidoctorの歴史
Asciidoctorは2012年にGitHub開発者であるNick HengeveldがGitのウェブサイトに示されているgitのマニュアルページを生成するために作成したプロトタイプをベースに、同じくGitHub開発者であるRyan Waldronがプロジェクトを開始しました。 +
2013年にAsciidoctorがGitHubにおいて発表され、その後はオープンソースプロジェクトとしてGitHubでホストされています。 +
このため、GitHubではサイト上で.adocファイルのプレビューを行うことができ、READMEやWikiをAsciiDoc形式で記述することが可能です。

* link:https://asciidoctor.org/[Asciidoctor 公式サイト]

==== Asciidoctorの利用方法と本環境

詳しいマニュアルもありますが、本環境については以下のBlog記事をご参照ください。本環境はBlog記事の構成を発展させたものとなります。

* 公式マニュアル
** link:https://asciidoctor.org/docs/user-manual/[Asciidoctor Users manual]
* Blog記事
** link:https://tech-blog.cloud-config.jp/2020-03-17-recommend-asciidoc/[AsciiDocを利用してエンジニアのナレッジを上手に集約しよう]
** link:https://tech-blog.cloud-config.jp/2020-03-26-github-actions-asciidoc1/[GitHub Actionsを使って気軽にAsciiDocを出力しよう(1)]
** link:https://tech-blog.cloud-config.jp/2020-04-02-github-actions-asciidoc-2/[GitHub Actionsを使って気軽にAsciiDocを出力しよう(2)]
** link:https://tech-blog.cloud-config.jp/2020-04-09-github-actions-asciidoctor-action1/[GitHub ActionsのAsciiDoc用アクションの仕組み(1)]
** link:https://tech-blog.cloud-config.jp/2020-04-16-github-actions-asciidoc-2/[GitHub ActionsのAsciiDoc用アクションの仕組み(2)]
** link:https://tech-blog.cloud-config.jp/2020-04-16-github-actions-asciidoctor-docker/[GitHub Actionsで利用したいAsciidoctorの環境]
** link:https://tech-blog.cloud-config.jp/2020-04-15-github-actions-docker/[GitHub Actionsで利用したいDockerコンテナの基礎]
** link:https://tech-blog.cloud-config.jp/2020-04-22-github-actions-asciidoc-pdf1/[GitHub ActionsでAsciiDocをPDF出力しよう(1)]
** link:https://tech-blog.cloud-config.jp/2020-05-11-github-actions-asciidoc-pdf2/[GitHub ActionsでAsciiDocをPDF出力しよう(2)]

== 記述方法とルール

本ドキュメントはこちらを参照の上で記載をお願いします。

include::descriptionGuidelines.adoc[leveloffset=+2]

== 構成するファイル
ここでは当リポジトリに含まれるファイルについて説明します。

=== ルートディレクトリ

index.adoc:: AsciiDoc最上位のファイルです。実コンテンツをインクルードしている他にも出力に必要な設定を行なっています。
README.adoc:: 本章の実体となるファイルです。
descriptionGuidelines.adoc:: 本ドキュメント記載ルールを簡単にまとめたファイルです。
define-common.adoc:: index.adocに記載すると盆雑になる出力設定を記載しています。
define-literal.adoc:: 表現ゆれを防いだり、記載を容易にするための共通的な表現を定義しています。 +
こちらもindex.adocに記載すると盆雑になるため外部定義となっており、編集中のプレビューもできるように、各ファイルの最上部で条件インクルードされています。 +


=== /images ディレクトリ

画像ファイルを配置するディレクトリです。


include::./images/README.adoc[leveloffset=+3]

=== /docinfo ディレクトリ

HTML出力用のテーマをとなるHTMLを配置するディレクトリです。


include::./docinfo/README.adoc[leveloffset=+3]

=== /themes ディレクトリ

PDF出力用のテーマを配置するディレクトリです。


include::./themes/README.adoc[leveloffset=+3]

=== /.github/workflows ディレクトリ

ワークフローの処理を配置するディレクトリです。


include::./.github/workflows/README.adoc[leveloffset=+3]

=== /fonts ディレクトリ

PDF出力に必要なフォントファイルを配置するディレクトリです。

include::./fonts/README.adoc[leveloffset=+3]


=== /diagram-configs ディレクトリ

一部の標準で回避できない設定を行うファイルを配置するディレクトリです。

include::./diagram-configs/README.adoc[leveloffset=+3]

=== /azureFunctions ディレクトリ

Azure static web appsを利用するのに必要なディレクトリです。

include::./azureFunctions/README.adoc[leveloffset=+3]

=== /deletetool ディレクトリ

リンクされないpngやadocファイルを削除するシェルスクリプトを配置するディレクトリです。

include::./deletetool/README.adoc[leveloffset=+3]
